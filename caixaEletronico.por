...
funcao inicio()
{
	enquanto(verdadeiro){
		escreval("  |=======================================================================================================|")
		escreval("  |---                          Bem vindo ao banco Start, escolha uma opção                            ---|")
...
funcao acessarConta(){
	escreval("  |=== Primeiro é necessario fazer seu login digitando o numero da conta e a senha ===|")
...
funcao telaOperacaoBancaria(inteiro objConta){
	...
	enquanto(logado){
		limpa()
		 escreval("  |====================================================|   ")
	 	 escreva("  |--- Olá, Bem vindo de volta " + nome, "      ---| \n")
...
funcao cadeia CPFValido(){ 

		logico cpfValidar = falso
		inteiro cpfValido[11]
		inteiro mult1[] = {10,9,8,7,6,5,4,3,2} // calculo de cpf valido
		inteiro mult2[] = {11,10,9,8,7,6,5,4,3,2} // calculo de cpf valido
		inteiro soma = 0, resto
		cadeia cpf = "0"
		enquanto(cpfValidar == falso){
		escreva("\n|-> Para confirmar informe seu cpf(Um digito por vez): <-|\n")
		...
		leia(cpfValido[i])
			...
			se(cpfValido[0]==cpfValido[1] e cpfValido[1]==cpfValido[2] e cpfValido[2]==cpfValido[3] e cpfValido[3]==cpfValido[4] e
				cpfValido[4]==cpfValido[5] e cpfValido[5]==cpfValido[6] e cpfValido[6]==cpfValido[7] e cpfValido[7]==cpfValido[8] e
				cpfValido[8]==cpfValido[9] e cpfValido[9]==cpfValido[10]){
 					escreva("\n[CPF inválido!]\n")
		}senao{
			para(inteiro i=0; i < 9; i++){
			soma+= (cpfValido[i] * mult1[i])
			}	
			resto = (soma * 10) %11
			se(resto == 10){
				resto = 0	
			}
			se (resto != cpfValido[9]){
				escreva("\n[CPF inválido!]\n")
			}senao{
				soma = 0
				
				para(inteiro i=0; i < 10; i++){
				soma+= (cpfValido[i] * mult2[i])
				}	
					resto = (soma * 10) %11
				se(resto == 10){
					resto = 0	
				}
				se (resto != cpfValido[10]){
					escreva("\n[CPF inválido!]\n")
					}senao{
					escreva("\n[CPF Válido]\n")
					cpfValidar = verdadeiro
						}cpf = "" + cpfValido[0]+cpfValido[1] + cpfValido[2] + cpfValido[3] + cpfValido[4] + cpfValido[5] + 
						cpfValido[6] + cpfValido[7] + cpfValido[8] + cpfValido[9] + cpfValido[10]
 					}}
			}
			retorne cpf 
 ...
 * 
 * @POSICAO-CURSOR = 11736; 
 * @DOBRAMENTO-CODIGO = [12, 39, 45, 33, 59, 75, 81, 137, 168, 192, 197, 209, 221, 231, 241, 262, 274, 281, 296, 326, 313, 355, 361, 367, 394, 399, 405, 415];
 * @PONTOS-DE-PARADA = ;
